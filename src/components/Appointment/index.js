...
export default function Appointment(props) {
  ...
  return (
    <article className="appointment" data-testid="appointment">
      ...
      {mode === EDIT && <Form onCancel={back} onSave={save} name={props.interview.student} interviewer={props.interview.interviewer.id} interviewers={props.interviewers}/>}
      {mode === SAVING && <Status message={"Saving..."}/>}
      {mode === DELETING && <Status message={"Deleting..."}/>}
      {mode === CONFIRM && <Confirm message={"Are you sure you would like to delete?"} onCancel={back} onConfirm={destroy} />}
      {mode === ERROR_SAVE && <Error message={"Could not boook appointment"} />}
